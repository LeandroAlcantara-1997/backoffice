...
2. Crie um arquivo .env com as mesmas envs do env.example;
3. Execute make up para subir as imagens do rabbitmq e redis;
4. Execute make backoffice para construir e executar a imagem da aplicação;
5. O envio de mensagens pode ser realizado tanto com rabbitmq ui ou via script na pasta test;
...
* Foi utilizado abstração da implementação do RabbitMQ para evitar retrabalho caso a ferramenta de fila/stream mude para kafka ou sqs.
* Foi implementado tanto o consumer da fila task.in conforme requisito, quanto da task.out.
* No consumo de task.out foi adicionado a persistência da mensagem em um redis enviado por task.in.
* Além das filas de task.in e task.out, foi criada uma fila de dlq, no qual recebe toda mensagem recusada no processamento de task.in e task.out.
